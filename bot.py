...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "crypto_help":
        await query.edit_message_text("📈 Crypto example:\n/analyze BTCUSDT\n/analyze ETHUSDT", reply_markup=main_menu())
    elif data == "forex_help":
        await query.edit_message_text("📊 Forex example:\n/analyze EUR/USD forex\n/analyze XAU/USD forex\n\nRequires Twelve Data API key.", reply_markup=main_menu())
    elif data == "stock_help":
        await query.edit_message_text("🏦 Stock example:\n/analyze AAPL stock\n/analyze TSLA stock\n\nRequires Twelve Data API key.", reply_markup=main_menu())
    elif data == "news":
...
def fetch_binance_klines(symbol="BTCUSDT", interval="15m", limit=150):
...
def fetch_twelvedata(symbol: str, asset_type: str, interval="15min", outputsize=150):
    if not TWELVEDATA_API_KEY:
        raise ValueError("Twelve Data API key is missing. Add TWELVEDATA_API_KEY in .env")
